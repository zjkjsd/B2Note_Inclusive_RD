...
Two defects in the tight mask were identified while documenting it, and both
require a new reconstruction campaign to correct.

First, the intended requirement is that the acceptance and track-fit quality
apply to \emph{every} track, with only the impact-parameter requirement
depending on $p_{T}$.
...
conjunction}), so this parses as
\texttt{[A and B and C and [$p_{T}<0.15$ and \ldots]] or [\ldots] or \ldots}
and the acceptance and \texttt{pValue} requirements are applied only to the
lowest $p_{T}$ branch.
Tracks above $0.15$~GeV$/c$ enter the ROE with no acceptance or fit-quality
requirement at all.
Bracketing the disjunction restores the intended behaviour.

Second, the leading \texttt{nCDCHits>=0} is satisfied by every track and has no
effect.

\AnalysisTBD{both defects change the ROE composition, and therefore
$M_{\mathrm{bc}}^{\mathrm{ROE}}$, $\Delta E^{\mathrm{ROE}}$, the ROE charge and
multiplicity, and $\mmiss$.  Everything in this note derived from ROE
quantities --- including the generic-$B\bar{B}$ weights of
Sec.~\ref{sec:bbbar}, which are tuned partly on the ROE multiplicity ---
predates the fix and will need regenerating once the samples are reproduced}
