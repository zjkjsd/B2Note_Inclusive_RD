...
%   commit      : ad2f2ec (analysis repository, referenced by SHA only)
...
As an organisational template only: a sibling Belle~II note presents this
kind of check as, per particle species, one efficiency-versus-cut-value plot
and one fake-rate-versus-cut-value plot, overlaying the different available
identifiers (there, a likelihood-based and a neural-network-based score)
rather than a single point at the chosen working point --- useful as a
layout template for these plots once produced}
...
As an organisational template only, for whenever the ROE mask is revisited
beyond this bug fix: a sibling Belle~II note tunes its ROE mask by an
exhaustive scan over combinations of mask cut variables and thresholds,
ranking each combination by a signal-significance figure of merit on MC with
embedded signal, then breaking near-ties among the top candidates by
data/MC agreement of the ROE-derived variables, and finally checking that
the chosen mask remains competitive under an alternative,
uncertainty-weighted objective as a robustness check.  That three-step
pattern --- FOM scan, data/MC tie-break, robustness re-check --- is a
reusable template if the mask is ever re-optimised here rather than only
bug-fixed}
