...
weight inward while re-minimising the others.  A diagnostic that does this
exists on the \texttt{claude/bbbar-tuning-diagnostics} branch of the analysis repository but is not part of the analysis at
the pinned commit: repeated review found several ways for its verdict to be
confidently wrong, and it has never been run on real ntuples.  Everything
above is therefore an inference from the metadata of eight stored fits.  No
conclusion about merging decay families should be drawn until the scan has
been validated and run}
