...
\begin{enumerate}
  ...
  \item The tuning region cannot separate the unmeasured families, in which
        ...
        As an organisational reference only: a sibling Belle~II note
        ($B\to K^{(*)}\nu\bar\nu$) reports exactly this situation for an
        analogous background grouping, stating that its control region ``has
        no handle to constrain them separately,'' and its response is not to
        keep attempting an in-situ fit but to assign one shared,
        externally/PDG-constrained nuisance parameter per group instead ---
        a concrete precedent for the ``merge and constrain'' remedy in this
        item, should the scan below confirm this explanation over the third.
...
As an organisational reference only: a sibling Belle~II note
($B\to K^{(*)}\nu\bar\nu$) validated its full background-weight product in
an independent sideband and reports an imperfect but acceptable closure,
which it only reached after loosening one background normalisation by $50\%$
relative to its weighted value.  A validation attempt here that shows
tension rather than clean closure should not automatically be read as the
weights failing; it may indicate that a genuinely independent region needs
its own residual freedom, the same way that note's did}
