...
\InProgress{the three explanations are distinguished by
\texttt{scripts/scan\_bbbar\_bounds.py}, which re-runs the minimisation over a
sequence of bound relaxations, profiles each pinned weight inward, and reports
which of the three applies.  Everything above is an inference from the
metadata of eight stored fits, not the result of that scan; no conclusion
about merging families should be drawn until it has been run}
