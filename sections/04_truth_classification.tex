...
A more complete, though still partial, reference point is
Table~\ref{tab:sensitivity-sample-sizes} (Sec.~\ref{sec:fit:templates}):
reconstructed event counts for all fourteen categories, Run~1 and Run~2
combined, separately for $e$ and $\mu$.
It answers the channel gap of the two reference points above (both were
$e$-channel only) but not the run-period split, and its selection is a
different, tighter working point than the \texttt{offline\_cut}-only numbers
above (Sec.~\ref{sec:fit:templates} records this discrepancy in scale
explicitly rather than reconciling it).
\AnalysisTBD{table of expected yields and fractions per category, in the
signal region and in the $D$-mass sideband \emph{separately}, and split by
Run~1 versus Run~2, at the nominal working point.  None of the three
reference points now in the note is that table: each is a partial snapshot
from a different notebook run at a different selection stage}
